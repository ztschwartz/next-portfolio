...
const PaginationBtns = ({pageNumber}) => {
    ...
    const getNextPage = () => {
        ...
        if (nextPageNumber <= arrayLength){
            ...
            return [nextPageSlug, nextPageTitle]
        }else{
    ...
    const getPrevPage = () =>{
        ...
        if (prevPageNumber > 0 ){
            ...
            return [prevPageSlug, prevPageTitle]
        }else{
            console.log(arrayLength);
    ...
    const [nextPageSlug, nextPageTitle] = getNextPage();
    const [prevPageSlug, prevPageTitle] = getPrevPage();
